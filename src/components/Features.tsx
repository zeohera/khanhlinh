...
export default function Features({ data }: FeaturesProps) {
  const displayFeatures = data?.items || [
    {
      icon: '🛡️',
    ...
    <section className="features" id="features">
      <div className="container">
        <div className="features-header">
          <div className="section-badge">{data?.badge || '💎 Tại sao chọn chúng tôi'}</div>
          <h2 className="section-title">
            {data?.title || 'Chất Lượng Vượt Trội –'}{' '}
            <span className="highlight">{data?.titleHighlight || 'Tin Dùng Hàng Triệu Khách'}</span>
          </h2>
          <p className="section-subtitle" style={{ margin: '0 auto' }}>
            {data?.subtitle || 'Vali Hùng Phát được sản xuất tại nhà máy đạt chuẩn BSCI, xuất khẩu sang Mỹ, Anh, Nhật Bản – cam kết chất lượng quốc tế với giá Việt Nam.'}
          </p>
        </div>
